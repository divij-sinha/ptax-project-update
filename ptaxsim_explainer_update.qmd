...
small_school <- shp_bnd_all_schools %>%
    ...
    left_join(tax_bill_data, by = join_by(AGENCY_DESC == agency_name))
...
One of these districts is your local school district - `r render_name(small_school$AGENCY_DESC)`! A portion of your taxes is sent to the agency to ensure it can continue operations.
...
>
`r if(!"Board Of Education" == small_school$AGENCY_DESC) {"<!--"}`
...
`r if(!"Board Of Education" == small_school$AGENCY_DESC) {"-->"}`
`r if(!"Chicago Community College District 508" == small_school$AGENCY_DESC) {"<!--"}`
...
```{r init-map}
shp_bnd_all_schools %>%
    filter(AGENCY_DESC == small_school$AGENCY_DESC) %>%
    map_dist()
```

Now, let's look at the line item of the bill for `r render_name(render_pin14)` in `r year` for `r render_name(small_school$AGENCY_DESC)` -

```{r increment-tax-bill-1}
tax_bill_data %>%
    filter(agency_name == small_school$AGENCY_DESC) %>%
    render_bill(
        c("School")
    ) %>%
    tab_style(
    ...
    tab_style(
        style = cell_fill(color = css_colors[small_school$AGENCY_DESC]),
        locations = cells_body()
    )
```

For the purpose of operating `r render_name(small_school$AGENCY_DESC)`, in `r year`, it needs to collect `r small_school %>% pull(agency_total_ext) %>% label_dol_amt() %>% render_name(small_school$AGENCY_DESC)` in order to provide services.
...
But where can `r render_name(small_school$AGENCY_DESC)` raise the money from? From all the properties within the district boundaries!  
The total taxable value of all properties falling in `r render_name(small_school$AGENCY_DESC)` make up its **tax base**. 
For `r year`, this was `r small_school %>% pull(agency_total_eav) %>% label_dol_amt() %>% render_name(small_school$AGENCY_DESC)`
...
The cost of services of `r render_name(small_school$AGENCY_DESC)` is therefore divided amongst all the property owners, and `r render_name(small_school$AGENCY_DESC)` sets its agency tax rate equal to: 
...
#### How much do you owe `r render_name(small_school$AGENCY_DESC)`?
...
The previously calculated `r render_name("rate", small_school$AGENCY_DESC)` is then applied to your `r render_name("addition to the tax base", render_pin14)`:
...
This is the total tax amount you owe to `r render_name(small_school$AGENCY_DESC)`
