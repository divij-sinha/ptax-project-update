...
The tax bill for `r render_name(render_pin14)` was approximately `r label_dol_amt(prior_year_tax_bill_summary$sum_final_tax_to_dist)` in `r prior_year` and `r label_dol_amt(latter_year_tax_bill_summary$sum_final_tax_to_dist)` in `r current_year`, resulting in `r if (tax_bill_change<0) "a" else "an"` __`r change_str(tax_bill_change, 'word')`__ of `r render_name(label_dol_amt(abs(tax_bill_change)),change_str(tax_bill_change, 'past'))`. This change can be broken down into two main components as shown below. The largest component of this change was [eav/commercial/levy/exemptions].
...
A change in exemptions between `r prior_year` and `r current_year` caused your tax bill to __`r change_str(total_exe_incident, 'word')`__ by  `r render_name(label_dol_amt(abs(total_exe_incident)), change_str(total_exe_incident, 'past'))`.  
...
::: {.column-margin}
Your property's fair market value (as determined by the Cook County Assessor's Office) is `r label_dol_amt(market_value)`. This is used to calculate your property's taxable value, called the Equalized Assessed Value (EAV). This year, your EAV was `r label_dol_amt(eav_current)`. See [exix A](#appendixA) for more details about this calculation. 
:::
...
See the breakdown of the change in EAV Residential Share for each district in [Appendix D](appendix)
:::
...
See the breakdown of the change in Commercial Share for each district in [Appendix D](appendix)
:::
...
::: {.callout-note appearance="minimal"}
#### Appendix B: Calculating a Single District's Tax Bill {#appendixB}
...
::: {.callout-note appearance="minimal"}
#### Appendix C: Properties Tax Districts {#appendixC}
...
::: {.callout-note appearance="minimal"}
#### Appendix D: Breakdown of Change by District {#appendixD}
