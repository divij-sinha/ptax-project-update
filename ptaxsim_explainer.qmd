...
The tax bill for `r render_name(render_pin14)` was approximately `r label_dol_amt(prior_year_tax_bill_summary$sum_final_tax_to_dist)` in `r prior_year` and `r label_dol_amt(latter_year_tax_bill_summary$sum_final_tax_to_dist)` in `r current_year`, resulting in `r if (tax_bill_change<0) "a" else "an"` __`r change_str(tax_bill_change, 'word')`__ of __`r label_dol_amt(abs(tax_bill_change))`__. This change can be broken down into main components as shown below.

```{r}
change_from_levy_text <- paste("Your tax bill ", change_str(change_from_levy, "past"), " by approximately __", label_dol_amt(abs(change_from_levy)), "__ due to the change in your taxing districts' levies. ",
    "This is the change in the tax bill outside your control, is uniformly felt by everyone, and refers to the increase in the cost of providing services.",
    sep = ""
)
change_from_share_text <- paste("Your tax bill ", change_str(change_from_share, "past"), " by approximately __", label_dol_amt(abs(change_from_share)), "__ due to your property's assessed value changing differently than other properties in your area. ",
    "This is the change in the tax bill that is based on your property value, and represents the change in your relative burden of property taxes compared to your neighbours",
    sep = ""
)
...
[^exemptionnote]:  
    ...
    This property currently has the following exemptions applied - 
    `r if(pin_exe_total == 0) {"-->"}`
    `r if(pin_exe_total != 0) {"<!--"}`  
    ...
    pin_exe_details %>%
        select(name, value) %>%
        mutate(p_signifier = fontawesome::fa("check","green")) %>%
        mutate(n_signifier = fontawesome::fa("xmark","red")) %>%
        mutate(signifier = ifelse(value > 0, p_signifier, n_signifier)) %>%
        mutate(signifier = map(signifier, html)) %>%
        select(!c(p_signifier, n_signifier)) %>%
        gt() %>%
        cols_label(
            name = "Exemption Type",
            signifier = "Exemption Active",
