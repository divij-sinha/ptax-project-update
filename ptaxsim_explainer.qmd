...
```{r}
levy_y = (total_levy_current-total_levy_prior)/total_levy_prior

share_current = sum(tax_bill_current$tax_amt_post_exe)/sum(tax_bill_current$agency_total_ext)
share_prior = sum(tax_bill_prior$tax_amt_post_exe)/sum(tax_bill_prior$agency_total_ext)

share_x = (share_current - share_prior)/ share_prior

y_min =  -1.2*abs(levy_y)
y_max = 1.2*abs(levy_y)

x_min = -1.2*abs(share_x)
x_max = 1.2*abs(share_x)

pin_date = data.frame(x = c(share_x), y = c(levy_y))

if (y_max < x_max) {
  line_data <- data.frame(x = c(-y_max, -y_min), y = c(y_max, y_min))
} else {
  line_data <- data.frame(x = c(x_min, x_max), y = c(-x_min, -x_max))
}

x_axis <- data.frame(x = c(x_min, x_max), y = c(0,0))
y_axis <- data.frame(x = c(0, 0), y = c(y_min*.9, y_max*.9))
...
quadrant_chart = ggplot() +
 ...
 geom_ribbon(data = line_data, aes(x = x, ymin = y, ymax = y_max), fill = "#ff696144", alpha = 0.1) +  # Shade area above the line
  geom_ribbon(data = line_data, aes(x = x, ymin = y_min, ymax = y), fill = "#61ffb844", alpha = 0.1) +  # Shade area below the line
  ...
  annotate("text", x = x_max*.98, y = y_max*0.05, label = "Increased Tax Burden",fontface = "bold", hjust = 1) +  # Label at the end of positive x-axis
  annotate("text", x = x_min*.98, y = y_max*0.05, label = "Decreased Tax Burden",fontface = "bold", hjust = 0) +  # Label at the end of negative x-axis
  annotate("text", x = 0, y = y_max*.95, label = "Increased Levy", fontface = "bold") + # Label at the top of x-axis
  annotate("text", x = 0, y = y_min*.95, label = "Decreased Levy", fontface = "bold") +  # Label at the top of x-axis
  annotate("text", x = x_max*.5, y = y_max*.5, label = "Tax Bill Increases") +
  annotate("text", x = x_min*.5, y = y_min*.5, label = "Tax Bill Decreases") 
```
...
## My Property (TODO) (where to get parcel metadata)

```{r, eval=FALSE}
#This is `r render_name("property parcel", render_pin14)` which is located at `r pin_address`. It was last assessed in `r last_assessed_year` at a fair market value of `r fmv`. According to the Accessor's Office, this property is classified as a `r pin_details` -->
