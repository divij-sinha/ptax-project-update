...
Every `r render_name("PIN", render_pin14)` belongs to multiple tax districts (local government bodies), that each require funding for purposes like operating schools, libraries, parks etc.
...
Each year you receive one tax bill that is split across two payments. The first installment is due by March 1st and will be 55% of last year's tax bill. The second installment is sent around June 30 and is for the remaining taxes due.
...
This is `r render_name(render_pin14)` which is located at `r address`. See additional details below. 
...
The tax bill for `r render_name(render_pin14)` was approximately `r label_dol_amt(prior_year_tax_bill_summary$sum_final_tax_to_dist)` in `r prior_year` and `r label_dol_amt(latter_year_tax_bill_summary$sum_final_tax_to_dist)` in `r current_year`, resulting in `r if (tax_bill_change<0) "a" else "an"` __`r change_str(tax_bill_change, 'word')`__ of `r render_name(label_dol_amt(abs(tax_bill_change)),change_str(tax_bill_change, 'past'))`. This change can be broken down into two main components as shown below. The largest change in your bill was due to __`r biggest_change`__.
...
Your tax levy `r change_str(change_from_levy, 'past')` and your assessed value `r change_str(change_from_share, 'past')`, while your combined tax bill `r change_str(tax_bill_change, 'past')`. 
...
## [How did your property's change in price affect my bill?]{style="color: #011E56;"}

This share is how much your property’s taxable value, or its Equalized Assessed Value, makes up of the the taxable value of all properties, called the Tax Base. The Tax Base is the combination of the taxable value of all residential and commercial properties.
...
<div class="box"> <b>(A) Change from Exemptions</b></div>
  <div class="box"><b>(B) Change from your Property's Share of all Residential Properties</b></div>
  <div class="box"> <b>(C) Change from Commercial-Residential Ratio</b></div>
  
</div>

#### (A) Change from Exemptions

Your tax bill may have changed due to a change in your exemptions. Property tax exemptions are savings that contribute to lowering a homeowner’s property tax bill. Learn more about exemptions, including how to apply, at the [Cook County Assessor's Office's website](https://www.cookcountyaa.gov/property-tax-exemptions). 
...
A third reason part your tax bill changed is due to how much __all__ residential properties pay compared to __all__ commercial properties. Commercial properties are more likely to decrease their assessment value through appeals, which shifts the tax burden more onto residential properties. The share that all residential properties paid in `r current_year` `r change_str(total_res_com_share_incident, 'past')` caused your tax bill to __`r change_str(total_res_com_share_incident, "word")`__ by `r render_name(label_dol_amt(abs(total_res_com_share_incident)),change_str(total_res_com_share_incident, "past"))`
...
::: {.column-margin}
See the break down of the tax levy change for each distinct in [Appendix C](#sec-appendixC)
:::
