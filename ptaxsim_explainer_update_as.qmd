...
pin_data = dbGetQuery(ptaxsim_db_conn, "
                        SELECT *, 
                            case when 
                            substr(class,1,1) IN ('2','3','4') then 'res'
                            else 'com' end as type
                        FROM pin 

                        WHERE substr(class,1,1) IN ('2','3','4','5') 
                        order BY RANDOM()
                        limit 100000")
...
#TODO i must be calculating EAV wrong
...
tax_bill_1a_chi <- tax_bill_1a %>%
    filter(agency_num == chicago_agency_num) 
tax_bill_prior <- tax_bill_1a %>%
    filter(agency_num == chicago_agency_num) 

# TODO is this correct
eav_res_share_incident = tax_bill_1a_chi$tax_amt_post_exe - tax_bill_prior$tax_amt_post_exe
```
...
tax_bill_1b_chi <- tax_bill_1b %>%
    filter(agency_num == chicago_agency_num) 
tax_bill_prior <- tax_bill_1b %>%
    filter(agency_num == chicago_agency_num) 

# TODO is this correct
res_com_share_incident = tax_bill_1b_chi$tax_amt_post_exe - tax_bill_prior$tax_amt_post_exe
```
...
An `r change_str(change_from_levy, 'word')` of __`r abs(change_from_levy) %>% label_dol_amt`__ or __`r change_from_levy / sum(tax_bill_data_prior_year$tax_amt_post_exe)`__ was due to the change in your taxing districts' levies.
:::

::: {.callout-warning appearance="minimal"}
A `r change_str(change_from_share, 'word')` of __`r abs(change_from_share) %>% label_dol_amt `__ or __`r change_from_share / sum(tax_bill_data_prior_year$tax_amt_post_exe)`__ was due to your property's assessed value going up more quickly than other properties in your area.
:::
...
![tax_bill_fig](tax_bill_fig.png){fig-align="center"}
...
One part of your tax bill changing is due to the change in your property's value compared to all other properties in your area. Your property's taxable value changed by `r eav_current-eav_prior` however, because the total taxable value of all residential properties also changed by `r res_base_current - res_base_prior`, your share of the total residential base changed from `r eav_res_share_prior` to `r eav_res_share_current`. This caused your tax bill to change by `r eav_res_share_incident`.
...
```{r appendix_a}
market_value = pin_details_current$av/.1
