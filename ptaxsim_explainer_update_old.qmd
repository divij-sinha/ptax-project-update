...
pin_14 <- as.character(params$pin_14)
...
pin_details_prior_year <- lookup_pin(prior_year, pin_14)
...
tax_bill_data_prior_year <- tax_bill(
    year_vec = prior_year,
    pin_vec = pin_14,
    simplify = F
) %>% process_tax_bill()

current_year_agency <- lookup_agency(year = year, tax_code = lookup_tax_code(year = year, pin = pin_14))
...
other_schools <- shp_bnd_all_schools %>%
    filter(!AGENCY_DESC %in% first_school$AGENCY_DESC)
...
One of these districts is your local school district - `r render_name(first_school$agency_name)`! A portion of your taxes is sent to the agency to ensure it can continue operations.
...
Now, let's look at the line item of the bill for `r render_name(render_pin14)` in `r year` for `r render_name(first_school$agency_name)` -
...
For the purpose of operating `r render_name(first_school$agency_name)`, in `r year`, it needs to collect `r first_school %>% pull(agency_total_ext) %>% label_dol_amt() %>% render_name(first_school$agency_name)` in order to provide services.
...
For `r year`, this was `r first_school %>% pull(agency_total_eav) %>% label_dol_amt() %>% render_name(first_school$agency_name)`
...
$$
(`r label_dol_amt(pin_details$eav - pin_exe_total)` * `r label_p_ltx(first_school$agency_tax_rate)`) = `r label_dol_amt((pin_details$eav - pin_exe_total)*first_school$agency_tax_rate)`
$$
...
## Prior Tax Bill Comparison

Compared to this, your final, total tax bill, for `r render_name(render_pin14)` for the year `r year-1` looks as follows 

```{r}
tax_bill_data_prior_year %>%
    replace(is.na(.), 0) %>%
    ...
    add_row(
        agency_major_type = "TIF",
        agency_name = tax_bill_data_prior_year %>%
            select(tif_agency_name) %>%
            unique() %>% pull(),
        final_tax = ifelse(sum(tax_bill_data_prior_year$rpm_tif_to_rpm) > 0, sum(tax_bill_data_prior_year$rpm_tif_to_rpm), sum(tax_bill_data_prior_year$final_tax_to_tif))
    ) %>%
    filter(final_tax > 0) %>%
    select(agency_major_type, agency_name, final_tax) %>%
    group_by(agency_major_type) %>%
...
```{r}
if (sum(tax_bill_data$tax_amt_post_exe) > sum(tax_bill_data_prior_year$tax_amt_post_exe)) {
    bill_inc <- TRUE
    bill_change_text <- "increased"
} else {
    bill_inc <- FALSE
    bill_change_text <- "decreased"
}

```

That is, your tax bill `r {render_name(bill_change_text)}` by `r (sum(tax_bill_data$tax_amt_post_exe) - sum(tax_bill_data_prior_year$tax_amt_post_exe)) %>% abs() %>% label_dol_amt() %>% render_name(bill_change_text)` from `r prior_year` to `r year`

Given what we now know about how property taxes are calculated, let us break out the change into the a few different sections.

### Change due to change in the EAV

```{r}
if (pin_details$eav > pin_details_prior_year$eav) {
    eav_inc <- TRUE
    eav_change_text <- "increased"
} else if (pin_details$eav < pin_details_prior_year$eav) {
    eav_inc <- FALSE
    eav_change_text <- "decreased"
} else {
    eav_change_text <- "stayed the same"
}

```

For `r render_name(render_pin14)`, the EAV in **`r prior_year`** was `r pin_details_prior_year %>% pull(eav) %>% label_dol_amt() %>% render_name(render_pin14)`, and the EAV in **`r year`** was `r pin_details %>% pull(eav) %>% label_dol_amt() %>% render_name(render_pin14)`.

That is, the EAV `r {render_name(eav_change_text)}` by `r (pin_details_prior_year$eav - pin_details$eav) %>% abs() %>% label_dol_amt() %>% render_name(eav_change_text)`, or about `r ((pin_details_prior_year$eav - pin_details$eav) %>% abs() / pin_details$eav) %>% label_p_ltx() %>% render_name(eav_change_text)`

Due to just the change in EAV, the tax bill amount also changes! If the EAV from **`r year`**, `r pin_details %>% pull(eav) %>% label_dol_amt() %>% render_name(render_pin14)` was instead applied to the tax bill for **`r prior_year`**, the new tax bill would look as follows -- 

```{r}
if (sum(tax_bill_data_prior_year$tax_amt_post_exe) < sum(tax_bill_data_current_eav_prior_levy$tax_amt_post_exe)) {
    tax_amt_inc <- TRUE
    tax_amt_change_text <- "increased"
} else {
    tax_amt_inc <- FALSE
    tax_amt_change_text <- "decreased"
}

display_two_bills(tax_bill_data_prior_year, tax_bill_data_current_eav_prior_levy, "Tax amount (prior year)", "Tax amount (prior year, current EAV)")
```


That is, the total tax bill would have `r render_name(tax_amt_change_text)` from **`r sum(tax_bill_data_prior_year$tax_amt_post_exe) %>% label_dol_amt`** to **`r sum(tax_bill_data_current_eav_prior_levy$tax_amt_post_exe) %>% label_dol_amt`**. A change of `r abs(sum(tax_bill_data_current_eav_prior_levy$tax_amt_post_exe) - sum(tax_bill_data_prior_year$tax_amt_post_exe)) %>% label_dol_amt %>% render_name(tax_amt_change_text)` or `r (abs(sum(tax_bill_data_current_eav_prior_levy$tax_amt_post_exe) - sum(tax_bill_data_prior_year$tax_amt_post_exe)) / sum(tax_bill_data_prior_year$tax_amt_post_exe)) %>% label_p_ltx %>% render_name(tax_amt_change_text)` due to the change in your PIN's EAV.  The reason this change is not the same as the change in the EAV is due to the other PINs being reassessed and their EAV changing as well, leading to a different load on your PIN!

### Change due to change in levy

Now, let's instead focus on the levy being changed!  

Assuming your PIN's EAV stayed the same as in `r prior_year`, at `r pin_details_prior_year %>% pull(eav) %>% label_dol_amt() %>% render_name(render_pin14)`, but instead of last year's levies, the agencies had this year's levies, what would the total bill look like?

```{r}
if (sum(tax_bill_data_prior_year$tax_amt_post_exe) < sum(tax_bill_data_prior_eav_current_levy$tax_amt_post_exe)) {
    tax_amt_inc <- TRUE
    tax_amt_change_text <- "increased"
} else {
    tax_amt_inc <- FALSE
...
display_two_bills(tax_bill_data_prior_year, tax_bill_data_prior_eav_current_levy, "Tax amount (prior year)", "Tax amount (current year, prior EAV)")
```


That is, the total tax bill would have `r render_name(tax_amt_change_text)` from **`r sum(tax_bill_data_prior_year$tax_amt_post_exe) %>% label_dol_amt`** to **`r sum(tax_bill_data_prior_eav_current_levy$tax_amt_post_exe) %>% label_dol_amt`**. A change of `r abs(sum(tax_bill_data_prior_eav_current_levy$tax_amt_post_exe) - sum(tax_bill_data_prior_year$tax_amt_post_exe)) %>% label_dol_amt %>% render_name(tax_amt_change_text)` or `r (abs(sum(tax_bill_data_prior_eav_current_levy$tax_amt_post_exe) - sum(tax_bill_data_prior_year$tax_amt_post_exe)) / sum(tax_bill_data_prior_year$tax_amt_post_exe)) %>% label_p_ltx %>% render_name(tax_amt_change_text)` due to the change in your taxing districts' levies. 

### Putting the increases together

Therefore, we can now see that the summary of the tax bills look as follows - 

```{r}
tibble(
    Category = c("Prior Levy", "Current Levy"),
    Prior_EAV = c(
        sum(tax_bill_data_prior_year$tax_amt_post_exe) %>% label_dol_amt(),
        sum(tax_bill_data_prior_eav_current_levy$tax_amt_post_exe) %>% label_dol_amt()
    ),
    Current_EAV = c(
        sum(tax_bill_data_current_eav_prior_levy$tax_amt_post_exe) %>% label_dol_amt(),
        sum(tax_bill_data$tax_amt_post_exe) %>% label_dol_amt()
    )
) %>%
    gt() %>%
    cols_label(
        Category = "",
        Prior_EAV = "Prior EAV",
        Current_EAV = "Current EAV"
    ) %>%
    tab_style(
        style = list(
            cell_text(weight = "bold"),
            cell_fill(color = css_colors[render_pin14])
        ),
        locations = cells_body(
            columns = c(2),
            rows = c(1)
...
If we look at only the changes from the prior bill, we can see the following --
...
inc_table <- tibble(
    ...
    Prior_EAV = c(
        NA,
        (sum(tax_bill_data_prior_eav_current_levy$tax_amt_post_exe) - sum(tax_bill_data_prior_year$tax_amt_post_exe))
    ),
    Current_EAV = c(
        (sum(tax_bill_data_current_eav_prior_levy$tax_amt_post_exe) - sum(tax_bill_data_prior_year$tax_amt_post_exe)),
        (sum(tax_bill_data$tax_amt_post_exe) - sum(tax_bill_data_prior_year$tax_amt_post_exe))
    )
)
...
inc_table %>%
    ...
    fmt_currency(columns = c(2,3)) %>%
    fmt_missing(missing_text = "-")
```

That is, of the total `r inc_table[[2, 3]] %>% label_dol_amt() %>% render_name(render_pin14)` change in the property tax bill from last year, `r inc_table[[1, 3]] %>% label_dol_amt() %>% render_name(render_pin14)` can be attributed to a change in the PIN's EAV and  `r inc_table[[2, 2]] %>% label_dol_amt() %>% render_name(render_pin14)` can be attributed to a change in the taxing district's levies. The remaining  `r (inc_table[[2, 3]] - inc_table[[2, 2]] - inc_table[[1, 3]]) %>% label_dol_amt() %>% render_name(render_pin14)` is due to a combination of the two changes.
